...
class KPMUpdate(BaseUpdate):
    sub_section_re = re.compile(r"^=== *((?:<s>)? *\[\[:?[^=]+?) *===$", flags=re.M)

    category_name = "Категория:Википедия:Незакрытые обсуждения переименования страниц"
    result_page_name = "Википедия:К переименованию"
    result_page_template = """{{{{/Шапка}}}}

== Текущие обсуждения ==

{{{{#invoke:RequestTable|TableByDate|header=Статьи, вынесенные на переименование|link=Википедия:К переименованию
...
"""
    ...
    def format_sub_section(self, section: SubSection) -> str:
        ...
        if section.result and "<s>" not in section.title:
            title = f"<s>{title}</s>"
        elif section.disputed_result:
            title = f"{{{{Оспоренный итог|{title}}}}}"
        elif section.preliminary_result:
            title = f"{{{{Предварительный итогг|{title}}}}}"
        return f"\n** {title}"
